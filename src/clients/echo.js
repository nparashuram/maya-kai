...
ws.on('message', (data, flags) => {
    var msg = {};
    try { msg = JSON.parse(data); } catch (e) { console.log(data)}
    if (msg.type === _.MSG_INIT) {
        ID = msg.ID;
        console.log('Started listening to messages as ', ID);
        return;
    }
    if (ID !== null && msg.type === _.MSG_EVENT && msg.origin !== ID) {
