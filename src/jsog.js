...
JSOG.encode = function (original, idProperty, refProperty) {
    ...
    sofar = {};
    var objectsWithJSOG_OBJECT_ID = [];

    idOf = function (obj) {
        if (!obj[JSOG_OBJECT_ID]) {
            obj[JSOG_OBJECT_ID] = "" + (nextId++);
            objectsWithJSOG_OBJECT_ID.push(obj);
        }
    ...
    var result = doEncode(original);
    for (var i = 0; i < objectsWithJSOG_OBJECT_ID.length; i++) {
        delete objectsWithJSOG_OBJECT_ID[i][JSOG_OBJECT_ID];
    }
